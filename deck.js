module.exports = class Deck {
  ...
  deal(num=1) {
    if (this.deck.length >= num && this.deck.length >= 0) {
      this.drawn.unshift(...this.deck.splice(this.deck.length-num, this.deck.length));
    } else {
  ...
  reset() {
    ...
    for (let suit in suits) {
      for (let value in values) {
        deck.push({'suitIndex': parseInt(suit), 'suit': suits[suit], 'value': values[value], 'valueIndex': parseInt(value)});
      }
    }
