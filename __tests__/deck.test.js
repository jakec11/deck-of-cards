...
test('If 1 card from the deck is drawn when not specifying number to deal', () => {
  ...
  expect(deck.drawn).toHaveLength(1);
});
...
test('If any number passed into deal, deals the number of cards', () => {
  ...
  expect(deck.drawn).toHaveLength(drawNumber);
});
...
test('Sort on an empty drawn returns an empty array', () => {
  const deck = new Deck();

  deck.sort();
  expect(deck.drawn).toEqual([]);
...
test('Drawn deck with multiple suits and values are sorted', () => {
  ...
  deck.sort();
  expect(deck.drawn).toEqual(testDrawnDeckSorted);
});
...
test('Same suits in a deck are sorted', () => {
  ...
  deck.sort();
  expect(deck.drawn).toEqual(suitsSorted);
});
...
test('Different suits but same card value', () => {
  ...
  deck.sort();
  expect(deck.drawn).toEqual(valueSorted);
});